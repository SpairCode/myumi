    ...
    const listItem = listArray.map((list, key) => 
      <li key={key} >
        <Row>
          ...
          <Col span={6}> <span className={styles.completeCircle} ></span> </Col>
          <Col span={6}> <Button disabled onClick={ () => this.operateList(key) } className={styles.operateButton} type="primary" size="small"> 操作 </Button> </Col>
        </Row>
      </li>
    )
    return (
